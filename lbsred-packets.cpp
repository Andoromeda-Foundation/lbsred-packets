...
public:
    ...
    EOSLIB_SERIALIZE(red, (id)(total_amount)(people_limit)(description)(ledger)(deadline))
...
public:
    ...
    typedef eosio::multi_index<N(red), red> red_index;
    red_index red_packages;
    ...
    void sent(const account_name sender, const uint64_t people_limit, const asset &total_amount)
    {
        ...
        red_packages.emplace(_self, [&](auto &package) {
    ...
    void take(const account_name taker, const account_name sender)
    {
        ...
        auto itr = red_packages.find(gameid);

        eosio_assert(itr != red_packages.end(), "Red package not found");

        /*
         * uint64_t amount = itr->take(taker);
